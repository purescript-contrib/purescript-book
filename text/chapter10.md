...
Here is one way to give JavaScript access PureScript's `show` so that the foreign code has a way to generate a `String` from any `Show`able type.
...
1. (Medium) The following data type represents a binary tree with values at the leaves:
     ...
     Consult the [argunaut docs](https://github.com/purescript-contrib/purescript-argonaut-codecs#generics) for instructions on how to do this.
